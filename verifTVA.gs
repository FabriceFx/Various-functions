 ...
 *
 * @param {string} numero  Le numéro de TVA (espaces tolérés).
 * @return {string}        "VALIDE" ou message d'erreur explicite.
 * @customfunction
 *
 * @example
 *   =verifTVA("FR 40 303656847")  → "VALIDE"
 *   =verifTVA("DE123456789")      → "VALIDE"
 *   =verifTVA("XX123")            → "INVALIDE — code pays « XX » non reconnu"
 */
function verifTVA(numero) {
  ...
  const clean = String(numero).toUpperCase().replace(/[\s.\-]/g, "");

  if (clean.length < 4) {
    return "INVALIDE — numéro trop court";
  }
  ...
  if (!FORMATS_TVA_[codePays].test(clean)) {
    return `INVALIDE — format incorrect pour ${codePays}`;
  }

  // Vérification spécifique de la clé pour les numéros français
  if (codePays === "FR") {
    const cle = clean.substring(2, 4);
    const siren = clean.substring(4);

    // Si la clé est entièrement numérique, on peut la vérifier
    if (/^\d{2}$/.test(cle)) {
      const cleAttendue = (12 + 3 * (parseInt(siren, 10) % 97)) % 97;
      const cleIndiquee = parseInt(cle, 10);

      if (cleIndiquee !== cleAttendue) {
        return `INVALIDE — clé de contrôle FR incorrecte (${cleIndiquee} au lieu de ${cleAttendue})`;
      }
    }
  }
