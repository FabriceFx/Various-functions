 ...
 * en excluant les week-ends (samedi, dimanche) et les jours fériés français.
 *
 * @param {Date|string} dateDebut  Date de début.
 * @param {Date|string} dateFin    Date de fin.
 * @return {number}                Nombre de jours ouvrés.
 * @customfunction
 *
 * @example
...
function joursOuvres(dateDebut, dateFin) {
  if (!dateDebut || !dateFin) return "Erreur: dates manquantes";
  ...
  // Inverser si la date de fin est avant la date de début
  ...
  // Précalculer les jours fériés pour les années concernées
